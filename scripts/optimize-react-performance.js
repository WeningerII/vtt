...
function optimizeComponent(filePath) {
  ...
  if (OPTIMIZATION_PATTERNS.memoComponents.includes(componentName)) {
    if (!content.includes(`memo(function ${componentName}`) && !content.includes(`memo(${componentName})`)) {
      ...
      if (exportConstPattern.test(content)) {
        content = content.replace(exportConstPattern, `export const ${componentName} = memo(($1) => {`);
      ...
      const exportFunctionPattern = new RegExp(`export function ${componentName}\\(([^)]*?)\\)`, 'g');
      if (exportFunctionPattern.test(content)) {
        content = content.replace(exportFunctionPattern, `export const ${componentName} = memo(function ${componentName}($1)`);
    ...
    if (matches && matches.length > 0) {
      matches.forEach(match => {
        ...
        if (!before.includes('useMemo')) {
          ...
          const assignmentPattern = new RegExp(`const (\\w+) = ${calculation.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`);
  ...
  if (modified) {
    ...
    if (hasReactImport) {
      ...
      if (importMatch) {
        ...
        content = content.replace(importPattern, `import { ${sortedImports.join(', ')} } from 'react'`);
      }
    }
  }
