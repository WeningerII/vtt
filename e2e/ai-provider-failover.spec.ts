...
// Define minimal local types to avoid static ESM imports from core during test compile
type AICapability = "text_generation" | "image_generation" | string;
...
test.describe("AI Provider Failover System", () => {
  ...
  test("should successfully use primary provider when available", async () => {
    ...
    aiProviderRegistry.registerProvider(new MockSuccessProvider(), {
      apiKey: "test",
      timeout: 30000,
      maxRetries: 3,
  ...
  test("should failover to secondary provider when primary fails", async () => {
    ...
    aiProviderRegistry.registerProvider(new MockFailureProvider(), {
      apiKey: "test",
      timeout: 30000,
      maxRetries: 1,
      rateLimitPerMinute: 60,
  ...
  test("should handle provider timeout gracefully", async () => {
    ...
    aiProviderRegistry.registerProvider(new MockSlowProvider(), {
      apiKey: "test",
      timeout: 1000,
      maxRetries: 1,
      rateLimitPerMinute: 60,
  ...
  test("should respect provider enable/disable flags", async () => {
    ...
    await expect(aiProviderRegistry.generateText("Test prompt")).rejects.toThrow(
  ...
  test("should automatically failover between real providers", async () => {
    ...
    if (!hasMultipleProviders) {
      test.skip(
        ...
        "Multiple AI providers not configured",
      );
    }
...
test.describe("Error Handling and Recovery", () => {
  test("should handle network errors gracefully", async () => {
    ...
    aiProviderRegistry.registerProvider(new MockNetworkErrorProvider(), {
      apiKey: "test",
      timeout: 30000,
      maxRetries: 2,
      rateLimitPerMinute: 60,
  ...
  test("should handle rate limiting appropriately", async () => {
    ...
    aiProviderRegistry.registerProvider(new MockRateLimitedProvider(), {
      apiKey: "test",
      timeout: 30000,
      maxRetries: 3,
      rateLimitPerMinute: 60,
