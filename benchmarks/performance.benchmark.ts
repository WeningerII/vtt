  ...
  try {
    ...
    if (criticalBenchmarks.length > 0) {
      ...
      criticalBenchmarks.forEach((result) => {
        console.log(
          `- ${result.name}: ${result.opsPerSecond.toFixed(0)} ops/sec (< 1000 threshold)`,
        );
      });
    } else {
