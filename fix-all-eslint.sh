...
echo "Fixing useless escapes..."
find . -name "*.ts" -o -name "*.tsx" | grep -v node_modules | xargs sed -i 's/\\\//\//g'

# Create a Node script to fix the complex issues
cat > fix-eslint-violations.js << 'EOF'
const fs = require('fs');
...
echo "Running violation fixer..."
node fix-eslint-violations.js

echo "Running ESLint auto-fix..."
npx eslint . --ext .ts,.tsx --fix --quiet

echo "Getting final status..."
npx eslint . --ext .ts,.tsx --format compact 2>&1 | tail -5
