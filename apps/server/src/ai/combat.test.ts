...
describe("CrucibleService", () => {
    ...
    it("should prioritize healing when character is low on health", async () => {
      const context = createTestContext({
        character: {
          ...
          class: "cleric",
          abilities: [{ id: "cure-wounds", name: "Cure Wounds", type: "action", healing: "1d8+3" }],
          resources: {
    ...
    it("should recommend taking cover when under ranged attack", async () => {
      const context = createTestContext({
        battlefield: {
          terrain: ["grass"],
          hazards: [],
          cover: [{ position: { x: -5, y: 0 }, type: "full", size: "medium" }],
          lighting: "bright" as const,
          weather: "clear",
        },
        enemies: [
  ...
  describe("simulateCombat", () => {
    it("should return a valid combat simulation result", async () => {
      const party = [{ id: "char-1", name: "Fighter", hp: 50, ac: 18, initiative: 10 }];
      const enemies = [{ id: "enemy-1", name: "Goblin", hp: 10, ac: 15, initiative: 8 }];
      const battlefield = {
        terrain: ["grass"],
        hazards: [],
        cover: [],
  ...
  describe("analyzeCharacterRole", () => {
    ...
    it("should identify healer role for clerics", () => {
      const character = {
        ...
        ac: 16,
        abilities: [{ id: "cure-wounds", name: "Cure Wounds", type: "action" }],
      };
    ...
    it("should identify damage dealer role for rogues", () => {
      const character = {
        ...
        hp: { current: 30, max: 30 },
        ac: 15,
        abilities: [{ id: "sneak-attack", name: "Sneak Attack", type: "action" }],
      };
  ...
  describe("calculateOptimalAOEPosition", () => {
    it("should find optimal position for AOE spell", () => {
      ...
      const allies = [{ id: "a1", position: { x: 0, y: 0 } }];
    ...
    it("should avoid allies when calculating AOE position", () => {
      ...
      const allies = [{ id: "a1", position: { x: 11, y: 10 } }];

      const position = service.calculateOptimalAOEPosition(enemies, allies, 15);

      expect(position).toBeDefined();
      // Position should be adjusted to avoid the ally
      const distToAlly = Math.sqrt(Math.pow(position.x - 11, 2) + Math.pow(position.y - 10, 2));
