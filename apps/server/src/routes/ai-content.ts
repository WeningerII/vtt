...
import { generateEncounterContent, generateCampaignContent, enhanceTextContent, parseJsonBody } from './ai-content-helpers';
...
async function generateQuestContent(_prompt: string, level: number, _duration: string): Promise<any> {
...
async function generateItemsContent(_prompt: string, count: number, _rarity: string): Promise<any[]> {
  ...
  for (let i = 0; i < count; i++) {
    items.push({
      ...
      value: _rarity === 'common' ? 50 : _rarity === 'uncommon' ? 200 : _rarity === 'rare' ? 1000 : 5000
    });
  }
