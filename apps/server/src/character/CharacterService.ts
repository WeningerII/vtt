...
import { Character, CreateCharacterRequest, UpdateCharacterRequest, CharacterTemplate, Ability, Skill } from './types';
...
export class CharacterService {
  ...
  async createCharacter(userId: string, request: CreateCharacterRequest): Promise<Character> {
    ...
    const abilities = this.calculateAbilities(request.abilities || templateData.abilities || this.getDefaultAbilities(request.race));
    ...
    const character: Character = {
      ...
      experience: 0,
      hitPoints: {
        current: this.calculateMaxHP(request.class, request.level || 1, abilities.CON?.modifier || 0),
    ...
    if (this.isSpellcaster(request.class)) {
      character.spellcasting = this.initializeSpellcasting(request.class, request.level || 1, abilities);
    }
   ...
   * Update character
   */
  async updateCharacter(characterId: string, userId: string, update: UpdateCharacterRequest): Promise<Character | null> {
    ...
    if (update.level) {
      ...
      if (update.level > oldLevel) {
        ...
        character.hitDice.current = Math.min(character.hitDice.current + (update.level - oldLevel), update.level);
      }
    }
  ...
  private calculateSkills(characterClass: string, abilities: Record<string, Ability>): Record<string, Skill> {
  ...
  private calculateSavingThrows(characterClass: string, abilities: Record<string, Ability>): Record<string, { proficient: boolean; value: number }> {
  ...
  private getClassHitDie(characterClass: string): string {
    switch (characterClass.toLowerCase()) {
      ...
      case 'fighter': case 'paladin': case 'ranger': return 'd10';
      case 'bard': case 'cleric': case 'druid': case 'monk': case 'rogue': case 'warlock': return 'd8';
      case 'sorcerer': case 'wizard': return 'd6';
      default: return 'd8';
    }
  }
  ...
  private getClassSkillProficiencies(characterClass: string): string[] {
    switch (characterClass.toLowerCase()) {
      case 'barbarian': return ['Animal Handling', 'Athletics'];
      case 'bard': return ['Deception', 'History', 'Investigation', 'Persuasion'];
      case 'cleric': return ['History', 'Medicine'];
      case 'druid': return ['Arcana', 'Medicine'];
      case 'fighter': return ['Acrobatics', 'Athletics'];
      case 'monk': return ['Acrobatics', 'Athletics'];
      case 'paladin': return ['Athletics', 'Religion'];
      case 'ranger': return ['Animal Handling', 'Athletics', 'Insight', 'Investigation', 'Nature', 'Perception', 'Stealth', 'Survival'];
      case 'rogue': return ['Acrobatics', 'Athletics', 'Deception', 'Insight', 'Intimidation', 'Investigation', 'Perception', 'Performance', 'Persuasion', 'Sleight of Hand', 'Stealth'];
      case 'sorcerer': return ['Arcana', 'Deception'];
      case 'warlock': return ['Arcana', 'Deception'];
  ...
  private getStartingEquipment(characterClass: string, _background: string): any[] {
    ...
    switch (characterClass.toLowerCase()) {
      case 'fighter':
        equipment.push(
          { id: uuidv4(), name: 'Longsword', type: 'weapon', quantity: 1, weight: 3, equipped: true } as any,
          { id: uuidv4(), name: 'Shield', type: 'armor', quantity: 1, weight: 6, equipped: true } as any
        );
        break;
      case 'wizard':
        equipment.push(
          { id: uuidv4(), name: 'Spellbook', type: 'misc', quantity: 1, weight: 3 },
          { id: uuidv4(), name: 'Quarterstaff', type: 'weapon', quantity: 1, weight: 4, equipped: true } as any
        );
        break;
      case 'rogue':
        equipment.push(
          { id: uuidv4(), name: 'Shortsword', type: 'weapon', quantity: 1, weight: 2, equipped: true } as any,
  ...
  private initializeSpellcasting(characterClass: string, level: number, abilities: Record<string, Ability>): any {
  ...
  private getSpellcastingAbility(characterClass: string): string {
    switch (characterClass.toLowerCase()) {
      ...
      case 'sorcerer': case 'bard': case 'paladin': case 'warlock': return 'CHA';
      default: return 'INT';
    }
  }

  private getSpellSlots(characterClass: string, level: number): Record<string, { max: number; current: number }> {
  ...
  private getCantripsKnown(characterClass: string, level: number): number {
    switch (characterClass.toLowerCase()) {
      case 'wizard': case 'sorcerer': case 'warlock': return level >= 4 ? 4 : 3;
      case 'bard': case 'cleric': case 'druid': return level >= 4 ? 3 : 2;
      default: return 0;
    }
  }
  ...
  private getSpellsKnown(characterClass: string, level: number): number {
    switch (characterClass.toLowerCase()) {
      case 'wizard': return 6; // Spellbook starts with 6 spells
      case 'sorcerer': case 'bard': case 'warlock': return level + 1;
      case 'ranger': return level >= 2 ? Math.floor(level / 2) + 1 : 0;
      case 'paladin': return level >= 2 ? Math.floor(level / 2) : 0;
      default: return 0;
    }
  }
