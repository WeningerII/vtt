...
export const CommonSchemas = {
  ...
  limitedInt: (min = 0, max = Number.MAX_SAFE_INTEGER) => z.number().int().min(min).max(max),
...
export const validateRequest = (schemas: {
  ...
  return async (ctx: Context, next?: () => Promise<void>) => {
    try {
      ...
      if (schemas.query) {
        if (ctx.req.url && ctx.req.url.length > 4096) {
          // 4KB URL limit
...
/**
 * Express-compatible request validation middleware
 * Validates req.body, req.query, req.params and headers using provided Zod schemas.
 * Uses Express-parsed body and returns 400 on errors.
 */
export const validateExpressRequest = (schemas: {
  body?: z.ZodSchema;
  query?: z.ZodSchema;
  params?: z.ZodSchema;
  headers?: z.ZodSchema;
  ...
  return (req: any, res: any, next: any) => {
    try {
      ...
      if (schemas.headers) {
        const validated = schemas.headers.parse(req.headers ?? {});
