...
export interface MetricsSnapshot {
  ...
  http: {
    ...
    response_times: Record<string, MetricData>;
  };
  business: Record<string, MetricData>;
  timestamp: number;
}
...
function mapFromEntries<T>(entries: Iterable<[string, T]>): Record<string, T> {
...
class MetricsCollector {
  ...
  getMetrics(): MetricsSnapshot {
    ...
    return {
      ...
      http: {
        requests: mapFromEntries(this.requestCounts),
        errors: mapFromEntries(this.errorCounts),
        response_times: mapFromEntries(this.metrics),
      },
  ...
  private getBusinessMetrics(): Record<string, MetricData> {
    const businessMetrics: Record<string, MetricData> = {};
  ...
  async runChecks(): Promise<HealthStatus> {
    ...
    for (const [name, checkFn] of this.checks.entries()) {
      ...
      try {
        ...
        const result = await Promise.race<HealthCheckResult | never>([checkFn(), timeoutPromise]);
