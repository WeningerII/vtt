...
export class RedisWebSocketAdapter extends EventEmitter {
  private pub: RedisClientInstance;
  private sub: RedisClientInstance;
  ...
  async disconnect(): Promise<void> {
    ...
    await Promise.all([this.pub.quit(), this.sub.quit(), this.client.quit()]);
  ...
  async registerSession(clientId: string, info: Partial<SessionInfo>): Promise<void> {
    ...
    const sessionData: SessionInfo = {
      ...
      ...(info.gameId ? { gameId: info.gameId } : {}),
    };
    ...
    if (session) {
      ...
      if (session.serverId === this.serverId) {
        ...
        await this.pub.publish(
          `${this.keyPrefix}server:${session.serverId}`,
