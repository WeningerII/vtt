...
export function generateAltText(
  ...
  switch (type) {
    case "map":
      return context?.name ? `Map: ${context.name}` : "Battle map for tabletop gaming";

    case "token":
      return context?.name ? `Game token: ${context.name}` : "Character or creature token";

    case "avatar":
      return context?.name ? `Avatar for ${context.name}` : "User avatar image";
...
export class ScreenReader {
   ...
   */
  static updateDescription(element: HTMLElement, descriptionId: string, description: string): void {
