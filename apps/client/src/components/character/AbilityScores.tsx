                ...
                <div className="text-center">
                  ...
                  {isEditing ? (
                    <Input
                      ...
                      max="30"
                      value={score}
                      onChange={(e) => updateAbilityScore(key as keyof Character['abilities'], parseInt(e.target.value) || 1)}
          ...
          {ABILITIES.map(({ key, shortLabel }) => {
            ...
            return (
              <div key={key} className="flex items-center gap-3 p-2 bg-bg-tertiary rounded border">
                <button
                  onClick={() => toggleSavingThrowProficiency(key as keyof Character['savingThrows'])}
                  disabled={!isEditing}
                  className={cn(
