...
export const SOUND_THEMES: Record<string, SoundTheme> = {
  fantasy: {
    ...
    sounds: {
      ...
      spellCast: ["/sounds/fantasy/spell-cast-1.mp3", "/sounds/fantasy/spell-cast-2.mp3"],
  ...
  scifi: {
    ...
    sounds: {
      diceRoll: ["/sounds/scifi/digital-roll-1.mp3", "/sounds/scifi/digital-roll-2.mp3"],
...
class SoundManager {
  ...
  private calculateVolume(type: "effects" | "ambient" = "effects"): number {
    ...
    const baseVolume =
      type === "ambient" ? this.settings.ambientVolume : this.settings.effectsVolume;
...
export function SoundProvider({ children }: { children: React.ReactNode }) {
  ...
  const playSound = useCallback(
    async (event: SoundEvent, volume?: number) => {
  ...
  return React.createElement(SoundContext.Provider, { value: contextValue }, children);
}
