...
export class JWTManager {
  ...
  async generateAccessToken(payload: Omit<JWTPayload, 'iat' | 'exp' | 'iss'>, expiresIn?: number): Promise<string> {
    const tokenPayload: JWTPayload = {
      ...payload,
      iat: Math.floor(Date.now() / 1000),
      exp: Math.floor(Date.now() / 1000) + (expiresIn || this.parseExpiry(this.config.accessTokenExpiry)),
