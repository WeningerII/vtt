...
export class ModdingFramework {
  ...
  public async loadMod(manifest: ModManifest, modPath: string): Promise<ModLoadResult> {
    ...
    try {
      ...
      if (!depResult.satisfied) {
        errors.push(
          ...depResult.missing.map((dep) => `Missing dependency: ${dep.id} ${dep.version}`),
        );
      }
  ...
  private validateManifest(manifest: ModManifest): {
    valid: boolean;
    errors: string[];
    warnings: string[];
  } {
    ...
    if (manifest.id && !/^[a-z][a-z0-9_-]*$/.test(manifest.id)) {
      errors.push(
        "Invalid mod ID format. Use lowercase letters, numbers, underscores, and hyphens only",
      );
    }
  ...
  private checkDependencies(dependencies: ModDependency[]): {
    satisfied: boolean;
  ...
  private generateLoadOrder(manifest: ModManifest): number {
    ...
    const coreOverrides = manifest.assets.filter(
      (asset) => asset.override && asset.override.startsWith("core/"),
    );
