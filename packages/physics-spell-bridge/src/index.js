...
export class PhysicsSpellBridge extends EventEmitter {
    ...
    applyTeleportEffect(teleport, targets, position) {
        ...
        for (const targetId of targets) {
            ...
            if (teleport.requiresLineOfSight) {
                const raycast = this.physicsWorld.raycast(token.position, { x: position.x - token.position.x, y: position.y - token.position.y }, teleport.range);
    ...
    getTargetsInArea(center, radius, height) {
        const targets = [];
        for (const [tokenId, token] of this.tokens.entries()) {
            const distance = Math.sqrt(Math.pow(token.position.x - center.x, 2) +
                Math.pow(token.position.y - center.y, 2));
            if (distance <= radius) {
                // Check line of sight using physics raycast
                const raycast = this.physicsWorld.raycast(center, {
