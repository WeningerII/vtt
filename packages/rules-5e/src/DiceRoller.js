...
export class DiceRoller {
    ...
    rollDiceRoll(diceRoll) {
        if (diceRoll.advantage && diceRoll.disadvantage) {
            ...
            if (diceRoll.count === 1) {
                ...
                const total = allRolls.reduce((sum, roll) => sum + roll, diceRoll.modifier);
                return {
                    total,
                    rolls: allRolls,
     ...
     */
    rollInitiative(modifier = 0, advantage, disadvantage) {
        const diceRoll = {
            count: 1,
            sides: 20,
            modifier,
     ...
     */
    rollAbilityCheck(modifier = 0, advantage, disadvantage) {
        return this.rollInitiative(modifier, advantage, disadvantage);
    }
    /**
     * Roll saving throw (1d20 + modifier)
     */
    rollSavingThrow(modifier = 0, advantage, disadvantage) {
        return this.rollInitiative(modifier, advantage, disadvantage);
    }
    /**
     * Roll attack roll (1d20 + modifier)
     */
    rollAttack(modifier = 0, advantage, disadvantage) {
        return this.rollInitiative(modifier, advantage, disadvantage);
    }
    /**
     * Roll damage with potential critical hit doubling
     */
    rollDamage(expression, critical = false) {
        const parsed = this.parseDiceExpression(expression);
        ...
        if (critical) {
            ...
            result = {
                ...
                total: result.total + criticalRolls.reduce((sum, roll) => sum + roll, 0),
