...
export class PerformanceMonitor extends EventEmitter {
  ...
  getReport(timeRange?: { start: number; end: number }): PerformanceReport {
    ...
    const filteredSamples = this.samples.filter(
      (s) => s.startTime >= range.start && (s.endTime || s.startTime) <= range.end,
    );
  ...
  private calculateSummary(metrics: PerformanceMetric[], _samples: PerformanceSample[]) {
    ...
    return {
      averageFrameTime:
        frameTimeMetrics.length > 0
          ? frameTimeMetrics.reduce((sum, m) => sum + m.value, 0) / frameTimeMetrics.length
          : 0,
      memoryUsage:
        memoryMetrics.length > 0 ? memoryMetrics[memoryMetrics.length - 1]?.value || 0 : 0,
      cpuUsage:
        cpuMetrics.length > 0
  ...
  private checkThresholds(metric: PerformanceMetric): void {
    if (
      metric.name === "frame_time" &&
      metric.value > this.config.performanceThresholds.frameTime
    ) {
      const recentSlowFrames = this.frameTimings
        .slice(-this.config.alertThresholds.consecutiveSlowFrames)
