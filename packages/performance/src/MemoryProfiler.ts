...
export class MemoryProfiler {
  ...
  private detectLeaks(): void {
    ...
    if (avgGrowth > leakThreshold) {
      const severity =
        avgGrowth > leakThreshold * 3 ? "high" : avgGrowth > leakThreshold * 2 ? "medium" : "low";
  ...
  return (req: any, res: any, next: any) => {
    ...
    res.on("finish", () => {
      ...
      if (memoryDiff > 10 * 1024 * 1024) {
        // 10MB threshold
        console.warn(
          `[MemoryProfiler] High memory usage for ${req.method} ${req.path}: ${(memoryDiff / 1024 / 1024).toFixed(2)} MB`,
        );
      }
    });
