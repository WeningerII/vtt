...
export class OptimizationEngine {
    ...
    return [
      {
        ...
        condition: (_metrics, state: SystemState) => {
          return (
            state.renderer.fps < this.config.thresholds.fpsTarget &&
...

      {
        ...
        condition: (_metrics, state: SystemState) => {
          return (
            state.physics.simulationTime > 16 && // Taking more than 16ms
...

      {
        ...
        condition: (_metrics, state: SystemState) => {
          return (
            state.network.messagesPerSecond > 50 &&
...

      {
        ...
        condition: (_metrics, state: SystemState) => {
          return (
            state.ecs.updateTime > 10 && // Taking more than 10ms
  ...
  async analyzePerformance(): Promise<{
    ...
    if (systemState.renderer.fps < this.config.thresholds.fpsTarget) {
      analysis.recommendations.push(
        "Frame rate below target - consider reducing render quality or object count",
      );
    }

    if (systemState.physics.simulationTime > 16) {
      analysis.recommendations.push(
        "Physics simulation taking >16ms - reduce body count or accuracy",
      );
    }

    if (systemState.network.latency > this.config.thresholds.networkLatency) {
      analysis.recommendations.push(
        "High network latency detected - enable message batching or compression",
      );
    }
  ...
  getOptimizationStats(): {
    ...
    const averageImpact =
      this.optimizationHistory.reduce((sum, h) => sum + h.impact, 0) / total || 0;
