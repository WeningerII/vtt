...
export interface GameEvent extends EventData {
    type: 'combat_start' | 'combat_end' | 'turn_start' | 'turn_end' | 'character_death' | 'spell_cast' | 'damage_dealt' | 'condition_applied' | 'movement' | 'interaction' | 'rule_triggered' | 'automation_executed' | 'content_generated' | 'script_executed' | 'session_created' | 'session_ended';
    entityId?: string;
    targetId?: string;
    position?: {
        x: number;
...
export interface ContentEvent extends EventData {
    type: 'asset_created' | 'asset_modified' | 'asset_deleted' | 'generation_requested' | 'generation_completed' | 'import_completed' | 'export_completed' | 'content_generated';
    assetId?: string;
    assetType?: string;
    context?: Record<string, any>;
    data: any;
...
export interface AIEvent extends EventData {
    type: 'decision_made' | 'behavior_changed' | 'goal_updated' | 'pathfinding_completed' | 'state_transition' | 'personality_modified' | 'memory_updated';
    entityId: string;
    context?: Record<string, any>;
    data: any;
}
export interface RuleEvent extends EventData {
    type: 'rule_evaluated' | 'effect_applied' | 'workflow_started' | 'workflow_completed' | 'condition_met' | 'trigger_activated' | 'automation_triggered';
    ruleId?: string;
    workflowId?: string;
    context?: Record<string, any>;
    data: any;
...
export declare class UnifiedEventBus {
     ...
     */
    on<T extends SystemEvent>(_eventType: string, _handler: EventHandler<T>, options?: {
        priority?: number;
        once?: boolean;
     ...
     */
    once<T extends SystemEvent>(_eventType: string, _handler: EventHandler<T>, options?: {
        priority?: number;
        filter?: (event: T) => boolean;
    }): string;
    /**
