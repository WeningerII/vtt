...
export interface TraitEffect {
  type: 'damage' | 'healing' | 'condition' | 'teleport' | 'summon' | 
        'regeneration' | 'resistance' | 'immunity' | 'vulnerability' | 
        'spell_cast' | 'multiattack' | 'grapple' | 'shove' | 'frightening_presence';
  damage?: {
    dice: string;
    type: string;
...
export interface TraitTrigger {
  event: 'start_turn' | 'end_turn' | 'take_damage' | 'below_half_hp' | 
         'first_bloodied' | 'death' | 'kill_creature' | 'initiative_20' | 
         'enter_lair' | 'custom';
  condition?: string;
...
export class MonsterAbilitiesEngine {
  ...
  executeMonsterAbility(
    ...
    if (trait.type === 'legendary_action') {
      ...
      if (!legendary || legendary.currentActions < cost) {
        return { success: false, error: 'Insufficient legendary actions', effects: [], actionsUsed: 0 };
      }
    ...
    if (trait.mechanicalEffect) {
      const effectResults = this.executeTraitEffect(trait.mechanicalEffect, monster, targets, trait, context);
  ...
  processLairActions(encounterId: string, context?: any): MonsterAbilityResult {
    ...
    const availableActions = lair.actions.filter(action => 
      !action.cost?.recharge || this.rollRecharge(action.cost.recharge)
    );
      ...
      case 'condition':
        if (effect.condition) {
          for (const target of targets) {
            ...
            if (applyCondition) {
              this.conditions.applyCondition(target, effect.condition.id, effect.condition.duration);
              results.push({
                type: 'condition',
                targets: [target],
  ...
  getAvailableAbilities(monsterId: string): {
    ...
    return {
      ...
      rechargeableStatus: rechargeMap ? Object.fromEntries(rechargeMap) : {} as Record<string, any>
    };
  }
...
export const COMMON_MONSTER_TRAITS: Record<string, MonsterTrait> = {
  pack_tactics: {
    id: 'pack_tactics',
    name: 'Pack Tactics',
    description: 'Has advantage on attack rolls against a creature if at least one ally is within 5 feet of the target',
    type: 'trait',
    mechanicalEffect: {
      type: 'damage',
  ...
  frightful_presence: {
    ...
    description: 'Each creature within 120 feet must make a Wisdom saving throw or be frightened for 1 minute',
    type: 'action',
    mechanicalEffect: {
      type: 'frightening_presence',
      frightening: {
