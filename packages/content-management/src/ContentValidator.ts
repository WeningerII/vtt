  ...
  private setupDefaultRules(): void {
    ...
    this.rules.set("mimetype", {
      ...
      validator: (context) => {
        ...
        if (
          this.policy.allowedMimeTypes.length > 0 &&
          !this.policy.allowedMimeTypes.includes(mimeType)
        ) {
          result.issues.push({
            type: "error",
...
export const DEFAULT_VTT_POLICY: ContentPolicy = {
  ...
  allowedTypes: ["image", "audio", "model", "map", "token", "scene"],
  allowedCategories: ["characters", "environments", "items", "effects", "ui", "system"],
  allowedMimeTypes: [
    // Images
    "image/jpeg",
    "image/png",
