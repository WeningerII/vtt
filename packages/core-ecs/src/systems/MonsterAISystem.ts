...
export class MonsterAISystem {
  ...
  constructor(combatStore: CombatStore, healthStore: HealthStore, statsStore: StatsStore, learningSystem?: AILearningSystem) {
  ...
  decideTurn(entityId: EntityId, allEntities: EntityId[]): AIDecision {
    ...
    if (potentialTargets.length > 0 && combat.actionPoints > 0) {
      ...
      const attackDecision = this.evaluateAttack(entityId, potentialTargets, adaptiveBehavior, healthPercent);
    ...
    if (monsterData.spellcasting && combat.actionPoints > 0) {
      ...
      const spellDecision = this.evaluateSpellcast(entityId, potentialTargets, adaptiveBehavior, healthPercent);
      if (spellDecision) decisions.push(spellDecision);
    }

    // Consider movement (tactical repositioning)
    availableActions.push('move');
    const moveDecision = this.evaluateMovement(entityId, potentialTargets, adaptiveBehavior, healthPercent);
    if (moveDecision) decisions.push(moveDecision);

    // Use learning system to potentially override decision
    if (availableActions.length > 0) {
      const learnedAction = this.learningSystem.getBestAction(entityId, currentState, availableActions);
    ...
    const bestDecision = sortedDecisions.length > 0 
      ? sortedDecisions[0]
  ...
  decideLegendaryAction(entityId: EntityId, allEntities: EntityId[]): AIDecision | null {
    ...
    if (!monsterData?.legendaryActions || !combat || !this.combatStore.hasLegendaryActions(entityId)) {
      return null;
    }
    ...
    const preferredAction = availableActions.find((action: LegendaryAction) => 
  ...
  private evaluateAttack(entityId: EntityId, targets: EntityId[], behavior: AIBehavior, healthPercent: number): AIDecision | null {
    if (targets.length === 0) return null;

    // Choose target based on AI behavior
    const targetId = this.selectBestTarget(entityId, targets, behavior);
    
    const priority = behavior.aggressiveness * 8 + 
                    (healthPercent > 0.3 ? 2 : -2) + 
  ...
  private evaluateSpellcast(entityId: EntityId, targets: EntityId[], behavior: AIBehavior, healthPercent: number): AIDecision | null {
    if (targets.length === 0) return null;

    const targetId = this.selectBestTarget(entityId, targets, behavior);
    
    const priority = behavior.spellPreference * 9 + 
                    behavior.intelligence * 3 +
                    (healthPercent < 0.5 ? 3 : 0); // Prefer spells when low health
  ...
  private evaluateMovement(entityId: EntityId, targets: EntityId[], behavior: AIBehavior, healthPercent: number): AIDecision | null {
    const movePriority = behavior.selfPreservation * (1 - healthPercent) * 6 +
                        behavior.intelligence * 2;
  ...
  private selectBestTarget(entityId: EntityId, targets: EntityId[], behavior: AIBehavior): EntityId {
