...
export class UserManager extends EventEmitter {
  ...
  async cleanupExpiredResets(): Promise<void> {
    const now = new Date();
    const expired = Array.from(this.passwordResets.entries()).filter(([, reset]) => reset.expiresAt < now);
  ...
  async cleanupExpiredVerifications(): Promise<void> {
    const now = new Date();
    const expired = Array.from(this.emailVerifications.entries()).filter(([, verification]) => verification.expiresAt < now);
