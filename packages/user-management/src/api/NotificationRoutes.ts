...
export class NotificationRoutes {
  ...
  private async getInAppNotifications(req: Request, res: Response): Promise<void> {
    try {
      ...
      const notifications = this.notificationManager
        .getInAppNotifications(user.id, includeRead)
        .slice(0, limit);
  ...
  private async sendTestEmail(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.notificationManager.sendEmail(user.id, "test-email", {
  ...
  private async handleUnsubscribe(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.notificationManager.updateUserPreferences(userId as string, {
  ...
  private async getNotificationHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      const history = await this.notificationManager.getNotificationHistory(user.id, {
        limit,
        offset,
        type,
      });
