...
export class AStar {
  private static calculateDistance(nodeA: PathfindingNode, nodeB: PathfindingNode, heuristic: string): number {
  ...
  public static findPath(
    ...
    while (openSet.length > 0 && iterations < opts.maxIterations) {
      ...
      for (const neighbor of neighbors) {
        ...
        const newMovementCostToNeighbor = currentNode.gCost + this.getMovementCost(currentNode, neighbor);
...
// Utility functions
export function worldToGrid(worldPos: Vector2, _cellSize: number, gridOffset: Vector2 = { x: 0, _y: 0 }): Vector2 {
  return {
...
export function gridToWorld(gridPos: Vector2, _cellSize: number, gridOffset: Vector2 = { x: 0, _y: 0 }): Vector2 {
  return {
...
export function simplifyPath(path: Vector2[], _tolerance: number = 1): Vector2[] {
  ...
  for (let i = 1; i < path.length - 1; i++) {
    ...
    const crossProduct = (next.x - prev.x) * (current.y - prev.y) - (next.y - prev.y) * (current.x - prev.x);
