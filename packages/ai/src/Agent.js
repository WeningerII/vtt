...
export class Agent {
    ...
    rememberExperience(event, outcome, importance = 5) {
        ...
        if (this.memory.experiences.length > 1000) {
            this.memory.experiences.sort((a, b) => (b.importance * 0.7 + (Date.now() - b.timestamp) * 0.3) -
    ...
    // Perception queries
    canSee(position) {
        const distance = Math.sqrt((position.x - this.position.x) ** 2 +
    ...
    getNearbyAgents(range) {
        ...
        return this.perception.nearbyAgents.filter(agent => {
            const distance = Math.sqrt((agent.position.x - this.position.x) ** 2 +
    ...
    getDistanceTo(position) {
        return Math.sqrt((position.x - this.position.x) ** 2 +
            (position.y - this.position.y) ** 2);
    }
    getAngleTo(position) {
...
export class AgentManager {
    ...
    getStats() {
        ...
        return {
            ...
            averageUpdateTime: activeAgents.reduce((sum, agent) => sum + agent.getStats().averageUpdateTime, 0) / Math.max(activeAgents.length, 1)
        };
    }
