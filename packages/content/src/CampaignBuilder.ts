...
export class CampaignBuilder extends EventEmitter {
  ...
  // Session management
  addSession(title: string, date: Date, summary: string = "", duration: number = 0): void {
    const session = {
      id: this.generateId(),
  ...
  generateCampaignReport(): {
    ...
    return {
      ...
      quests: {
        ...
        completionRate:
          this.campaign.quests.length > 0
            ? this.campaign.quests.filter((q) => q.status === "completed").length /
              this.campaign.quests.length
            : 0,
      },
      locations: {
