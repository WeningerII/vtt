...
export class SpellExecutionEngine {
  ...
  private executeEffect(effect: EffectPrimitive, ctx: ExecutionContext, index: number): EffectResult {
    switch (effect.type) {
      case 'damage':
  ...
  private executeDamageEffect(effect: DamageEffect, ctx: ExecutionContext, index: number): EffectResult {
      ...
      if (effect.savingThrow) {
        ...
        if (roll >= dc) {
          switch (effect.savingThrow.onSave) {
            case 'half': damage = Math.floor(damage / 2); break;
            case 'none': damage = 0; break;
            case 'negate': continue;
          }
        }
      }
  ...
  private executeHealingEffect(effect: HealingEffect, ctx: ExecutionContext, index: number): EffectResult {
    const validTargets = Array.from(ctx.environment.entities.values())
      .filter(entity => effect.targetFilter(entity, ctx))
  ...
  private executeMovementEffect(effect: MovementEffect, ctx: ExecutionContext, index: number): EffectResult {
    // Implementation for movement effects
    return { effectIndex: index, success: true, targets: [] };
  }

  private executeConditionEffect(effect: ConditionEffect, ctx: ExecutionContext, index: number): EffectResult {
    // Implementation for condition effects
    return { effectIndex: index, success: true, targets: [] };
  }

  private executeTransformationEffect(effect: TransformationEffect, ctx: ExecutionContext, index: number): EffectResult {
    // Implementation for transformation effects
    return { effectIndex: index, success: true, targets: [] };
  }

  private executeSummonEffect(effect: SummonEffect, ctx: ExecutionContext, index: number): EffectResult {
    // Implementation for summon effects
    return { effectIndex: index, success: true, targets: [] };
  }

  private executeGeometryEffect(effect: GeometryEffect, ctx: ExecutionContext, index: number): EffectResult {
    // Implementation for geometry effects
    return { effectIndex: index, success: true, targets: [] };
  }

  private executeTimeEffect(effect: TimeEffect, ctx: ExecutionContext, index: number): EffectResult {
    // Implementation for time effects
    return { effectIndex: index, success: true, targets: [] };
  }

  private executeInformationEffect(effect: InformationEffect, ctx: ExecutionContext, index: number): EffectResult {
  ...
  private applyScaling(spell: ComputationalSpell, level: number, ctx: ExecutionContext): ComputationalSpell {
  ...
  private applyPropertyModification(obj: any, property: string, operation: string, value: any): void {
