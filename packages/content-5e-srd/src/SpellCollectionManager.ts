...
export class SpellCollectionManager {
   ...
   */
  createCollection(
    data: Omit<SpellCollection, "id" | "createdAt" | "updatedAt" | "version">,
  ): SpellCollection {
  ...
  createCollectionFromSearch(
    ...
    return this.createCollection({
      name,
      description:
        options?.description || `Collection created from search: ${JSON.stringify(searchCriteria)}`,
      spellIds,
