...
export const SPELL_MATERIAL_REQUIREMENTS: Record<string, SpellMaterialRequirement> = {
  ...
  'light': {
    ...
    components: [],
    alternativeComponents: [
      [getComponent('firefly')],
      [getComponent('phosphorescent_moss')]
    ]
  },
  'mending': {
    spellId: 'mending',
  ...
  'alarm': {
    ...
    components: [],
    alternativeComponents: [
      [getComponent('tiny_bell')],
      [getComponent('fine_silver_wire')]
    ]
  },
  'animal_friendship': {
...
export class MaterialComponentValidator {
  static validateSpellComponents(
    ...
    const missing = requirement.components.filter(required => 
      !availableComponents.some(available => available.id === required.id)
    );
    ...
    if (requirement.alternativeComponents) {
      for (const alternative of requirement.alternativeComponents) {
        const altMissing = alternative.filter(required =>
          !availableComponents.some(available => available.id === required.id)
        );
  ...
  static getComponentCost(spellId: string): number {
    ...
    return requirement.components.reduce((_total, _component) => 
      total + (component.cost || 0), 0
    );
  }
  ...
  static consumeComponents(
    ...
    return availableComponents.filter(available => 
      !consumed.some(consumedComp => consumedComp.id === available.id)
    );
  }
}
