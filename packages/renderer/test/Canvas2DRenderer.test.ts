...
describe("Canvas2DRenderer", () => {
  ...
  describe("mouse interaction", () => {
    it("should handle wheel events for zooming", () => {
      ...
      const wheelHandler = addEventListenerMock.mock.calls.find(
        (_call: any[]) => call[0] === "wheel",
      )?.[1];
    ...
    it("should handle mouse events for panning", () => {
      ...
      const mouseDownHandler = addEventListenerMock.mock.calls.find(
        (_call: any[]) => call[0] === "mousedown",
      )?.[1];
      const mouseMoveHandler = addEventListenerMock.mock.calls.find(
        (_call: any[]) => call[0] === "mousemove",
      )?.[1];
      const mouseUpHandler = addEventListenerMock.mock.calls.find(
