...
export class ShadowManager {
  ...
  private createSpotShadowMap(light: Light, config: ShadowMapConfig): ShadowData {
    ...
    this.calculateSpotLightMatrices(
      light,
      lightViewMatrix,
      lightProjectionMatrix,
      shadowMatrix
  ...
  updateCascadedShadowMap(shadowData: ShadowData, camera: Camera): void {
    ...
    const splitDistances = this.calculateCascadeSplits(nearPlane, farPlane, cascadeCount, config.splitLambda!);
  ...
  private calculateCascadeSplits(near: number, far: number, cascadeCount: number, lambda: number): number[] {
    const splits: number[] = [];
    splits.push(near);
  ...
  private calculateDirectionalLightMatrices(
    ...
    const up = Math.abs(lightDirection[1]) > 0.99 ? vec3.fromValues(1, 0, 0) : vec3.fromValues(0, 1, 0);
  ...
  private calculateFrustumCorners(camera: Camera, near: number, far: number): vec3[] {
    ...
    // Near plane corners
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), nearCenter, right, -nearWidth), vec3.scale(vec3.create(), up, nearHeight))); // Top left
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), nearCenter, right, nearWidth), vec3.scale(vec3.create(), up, nearHeight)));  // Top right
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), nearCenter, right, -nearWidth), vec3.scale(vec3.create(), up, -nearHeight))); // Bottom left
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), nearCenter, right, nearWidth), vec3.scale(vec3.create(), up, -nearHeight)));  // Bottom right
    
    // Far plane corners
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), farCenter, right, -farWidth), vec3.scale(vec3.create(), up, farHeight))); // Top left
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), farCenter, right, farWidth), vec3.scale(vec3.create(), up, farHeight)));  // Top right
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), farCenter, right, -farWidth), vec3.scale(vec3.create(), up, -farHeight))); // Bottom left
    corners.push(vec3.add(vec3.create(), vec3.scaleAndAdd(vec3.create(), farCenter, right, farWidth), vec3.scale(vec3.create(), up, -farHeight)));  // Bottom right
  ...
  getStats() {
    ...
    return {
      ...
      averageResolution: this.shadowMaps.size > 0 ? 
        Array.from(this.shadowMaps.values()).reduce((_sum, _data) => sum + data.config.resolution, 0) / this.shadowMaps.size : 0
    };
  }
}
