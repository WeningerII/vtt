...
export class PerformanceMonitor {
  ...
  public endFrame(): void {
    ...
    if (this.gpuTimer) {
      ...
      if (available) {
        this.metrics.gpuTime = this.gl.getQueryParameter(this.gpuTimer, this.gl.QUERY_RESULT) / 1000000; // Convert to ms
      }
    }
  ...
  public getPerformanceIssues(): string[] {
    ...
    if (this.metrics.frameTime > thresholds.maxFrameTime) {
      issues.push(`High frame time: ${this.metrics.frameTime.toFixed(2)}ms > ${thresholds.maxFrameTime}ms`);
    }
    ...
    if (this.metrics.textureMemory > thresholds.maxTextureMemory) {
      issues.push(`High texture memory: ${(this.metrics.textureMemory / 1024 / 1024).toFixed(1)}MB > ${(thresholds.maxTextureMemory / 1024 / 1024).toFixed(1)}MB`);
    }
    
    if (this.metrics.bufferMemory > thresholds.maxBufferMemory) {
      issues.push(`High buffer memory: ${(this.metrics.bufferMemory / 1024 / 1024).toFixed(1)}MB > ${(thresholds.maxBufferMemory / 1024 / 1024).toFixed(1)}MB`);
    }
