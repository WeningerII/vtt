...
export class GLTFLoader implements AssetLoader<ModelData> {
  ...
  private parseGLTF(
    ...
    const sceneData =
      gltf.scenes && gltf.scenes[sceneIndex] ? gltf.scenes[sceneIndex] : { nodes: [] };
    ...
    return meshes.map((mesh) => {
      ...
      if (attributes.TEXCOORD_0 !== undefined) {
        ...
        const normals = attributes.NORMAL
          ? (this.readAccessorData(accessors[attributes.NORMAL]) as Float32Array)
          : null;
        const interleavedData: number[] = [];
  ...
  private getDrawMode(mode?: number): number {
    switch (mode) {
      ...
      case 2:
        return WebGL2RenderingContext.LINE_LOOP;
      case 3:
      ...
      case 6:
        return WebGL2RenderingContext.TRIANGLE_FAN;
      default:
        return WebGL2RenderingContext.TRIANGLES;
    }
  }
