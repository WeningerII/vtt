...
export class BiomeGenerator {
  ...
  generateBiome(x: number, y: number): BiomeType {
    const temperature = this.temperatureNoise.fbm(x, y, {
      seed: 0,
      octaves: 4,
      persistence: 0.5,
      scale: 0.01,
    });
    const humidity = this.humidityNoise.fbm(x, y, {
      seed: 0,
      octaves: 4,
      persistence: 0.5,
      scale: 0.008,
    });
    const elevation = this.elevationNoise.fbm(x, y, {
      seed: 0,
      octaves: 6,
      persistence: 0.6,
      scale: 0.005,
...
export class QuestGenerator {
  private questTemplates = {
    ...
    kill: {
      objectives: [
        "Eliminate {monster} terrorizing {location}",
        "Clear {location} of {monster_type}",
      ],
      complications: [
        "has minions",
        "is magically protected",
        "can only be harmed by {special_weapon}",
      ],
    ...
    escort: {
      objectives: [
        ...
        "Guide {group} through {dangerous_area}",
      ],
  ...
  private fillPlaceholders(quest: any): any {
    const placeholders = {
      ...
      "{dangerous_location}": () =>
        this.randomChoice(["cursed swamp", "haunted forest", "abandoned mine", "ancient ruins"]),
      "{curse}": () =>
        this.randomChoice(["eternal slumber", "stone transformation", "memory loss", "aging"]),
      "{special_weapon}": () =>
        this.randomChoice(["silver blade", "blessed weapon", "magic sword", "holy water"]),
      "{enemies}": () =>
        this.randomChoice(["bandits", "wild beasts", "rival adventurers", "cultists"]),
      ...
      "{event}": () =>
        this.randomChoice(["the great fire", "the vanishing", "the curse", "the betrayal"]),
    };
