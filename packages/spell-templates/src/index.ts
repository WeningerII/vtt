...
export const createConditionSpell = (config: {
  ...
  srd: {
    ...
    concentration: config.concentration || false,
    ritual: false,
    savingThrow: config.savingThrow ? { ability: config.savingThrow.ability as any, dc: 0 } : undefined,
    source: 'SRD 5.1'
  },
  enhanced: {
    id: config.id,
 ...
 */
export function applyTemplate(template: SpellTemplate, overrides: Partial<SpellTemplate> = {}): SpellTemplate {
  return {
