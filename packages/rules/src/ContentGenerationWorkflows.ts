...
export class ContentGenerationWorkflowEngine {
  ...
  async processEvent(eventType: string, eventData: any, context: RuleContext): Promise<void> {
    ...
    const matchingTriggers = Array.from(this.activeTriggers.values()).filter(
    ...
    try {
      ...
      if (result) {
        ...
        await globalEventBus.emit(
          ContentEvents.contentGenerated(rule.contentType, result.id || "unknown", result),
        );

        // Execute rule effects if any
        if (rule.effects) {
          await this.ruleEngine.executeEffects(rule.effects, {
            ...context,
  ...
  private async generateEncounter(params: any): Promise<any> {
    ...
    const environment =
      params.environment || params.context?.currentLocation?.environment || "dungeon";
