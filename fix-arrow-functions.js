...
files.forEach(file => {
  ...
  content = content.replace(/onChange=\{?\(\(\) = aria-label="[^"]*"[^>]*> ([^}]+)\}\)/g, 'onChange={() => $1}');
  ...
  content = content.replace(/on([A-Z][a-zA-Z]+)=\{?\(\(\) = aria-label="[^"]*"[^>]*> ([^}]+)\}\)/g, 'on$1={() => $2}');
  content = content.replace(/on([A-Z][a-zA-Z]+)=\{?\(\(\) = ([^>]+)> ([^}]+)\}\)/g, 'on$1={() => $3}');
